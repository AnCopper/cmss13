/mob/living/carbon/xenomorph/proc/build_resin(atom/target, thick = FALSE, message = TRUE, use_plasma = TRUE, add_build_mod = 1, blocked_by_nearby_humans = FALSE)
	if(!selected_resin)
	...
	if(blocked_by_nearby_humans && !can_remote_build_resin_at(current_turf))
		return SECRETE_RESIN_FAIL
